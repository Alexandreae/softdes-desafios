...
def lambda_handler(event, context):
    """
    função lambda_handler adiciona desafios
    """
    try:
        import numbers
...
def converte_data(orig):
    """
    função converte_data converte valores
    """
...
def get_quizes(user):
    """
    função get_quizes seleciona os quizzes disponíveis para o usuário
    """
...
def get_user_quiz(userid, quizid):
    """
    função get_user_quiz envia a resposta do quiz
    """
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
    cursor.execute(
        "SELECT sent,answer,result from USERQUIZ wh ere userid = '{0}' and quizid = {1} order by sent desc"
...
def set_user_quiz(userid, quizid, sent, answer, result):
    """
    função set_user_quiz insere a resposta do quiz
    """
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
    cursor.execute("insert into USERQUIZ(userid,quizid,sent,answer,result) values ('{0}',{1},'{2}','{3}','{4}');".format(
...
def get_quiz(id, user):
    """
    função get_quiz recebe um quiz
    """
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
    if user == 'admin' or user == 'fabioja':
...
def set_info(pwd, user):
    """
    função set_info faz o update da senha do usuário
    """
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
...
def get_info(user):
    """
    função get_info recebe a senha do usuário
    """
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
    cursor.execute(
...
def main():
    """
    função main roda o codigo main
    """
    msg = ''
    p = 1
    ...
    if request.method == 'POST' and 'ID' in request.args:
        ...
        if len(quiz) == 0:
            ...
            return render_template('index.html',
                                   username=auth.username(), challenges=challenges, p=p, msg=msg)
        quiz = quiz[0]
        if sent > quiz[2]:
    ...
    return render_template('index.html', username=auth.username(),
                           ...
                           p=p, msg=msg, expi=converte_data(quiz[0][2]))
...
def change():
    """
    função change troca a senha do usuário
    """
    ...
    return render_template('index.html', username=auth.username(),
                           challenges=get_quizes(auth.username()), p=p, msg=msg)


@app.route('/logout')
def logout():
    """
    função logout faz o logout do usuário
    """
...
def get_password(username):
    """
    função get_password retorna a função get_info
    """
...
def hash_pw(password):
    """
    função hash_pw faz o encoding da senha
    """
